...
export default function TextToSpeechView({ loading, error, sortConfig, handleSort, sortedData }) {
  if (loading)
    return (
  ...
  if (!sortedData?.length)
    return (
      <p className="text-center text-gray-500 py-10 text-lg">
        No Text-to-Speech data available.
      </p>
    );
  ...
  return (
    <>
      <h2 className="text-3xl font-bold text-gray-800 mb-6 border-b pb-2 border-indigo-100">
        🔊 Text-to-Speech Models — ELO Rankings
      </h2>
