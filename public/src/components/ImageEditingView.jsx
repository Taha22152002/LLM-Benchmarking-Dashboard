...
export default function ImageEditingView({ loading, error, sortConfig, handleSort, sortedData }) {
  if (loading)
    return (
      <p className="text-center text-gray-500 py-10 text-lg">Loading Image Editing models...</p>
    );
  ...
  if (!sortedData?.length)
    return <p className="text-center text-gray-500 py-10 text-lg">No Image Editing data available.</p>;

  return (
    <>
      <h2 className="text-3xl font-bold text-gray-800 mb-6 border-b pb-2 border-indigo-100">
        🖌️ Image Editing Models — ELO Rankings
      </h2>
